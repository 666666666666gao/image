...
The script is at `scripts/ryomc_image.py` relative to this SKILL.md. Resolve that path on the user's machine. Run its `generate` command with `--prompt` and `--model` set to the exact model ID selected for the current Codex chat. The local script cannot read the active chat model itself: if the exact ID is not available in the task context, ask the user to specify it before sending a billable request. Do not substitute a model from a saved default or Codex config. For editing, pass `--input-image` pointing to a local PNG, JPEG, or WebP. Use `--quality low|medium|high` only when the user specifies or it matters. The script prints JSON containing the absolute `image_path`; display the result in the conversation using Markdown image syntax with that absolute path.
